...
class HelpWatchdog(object):
    def __init__(self):
        ...
        self.plan_check_period = rospy#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class HelpWatchdog(object):
    def __init__(self):
        ...
        # If there IS a plan but robot hasn't moved for this long -> stuck -> ask for help
        self.stuck_timeout = rospy.get_param("~stuck_timeout_sec", 5.0)

        # Distance that counts as "we moved" (meters)
        self.min_move_dist = rospy.get_param("~min_move_dist_m", 0.03)
        ...
        # Pause window after we beep
        self._pause_until = rospy.Time(0)
        ...
        # Track pose from AMCL for stuck detection
        rospy.Subscriber(
        ...
        rospy.loginfo(
            "help_watchdog: started (no-plan-threshold=%.1fs, stuck-timeout=%.1fs, "
            "min-move=%.2fm, pause-after-help=%.1fs, beep=%d)",
    ...
    def _timer_cb(self, event):
        ...
        # If we're in a "pause" window after asking for help, do nothing
        ...
        if goal is None:
            # No active goal -> reset state
            self._no_plan_accum = 0.0
            # Also reset movement timer so we don't immediately think we're stuck later
            self._last_moving_time = now
            return
        ...
        if ok:
            # There IS a global plan. Reset "no plan" timer.
            ...
            if stuck_dt >= self.stuck_timeout:
                ...
                self.beep_help()
                # Pause further checks for a while so we effectively "wait 10 seconds"
                self._pause_until = now + rospy.Duration(self.pause_after_help)
                # Reset movement timer so we don't immediately re-trigger
                self._last_moving_time = now
        ...
        if self._no_plan_accum >= self.no_plan_threshold:
            ...
            self.beep_help()
            # Pause checks after asking for help
            self._pause_until = now + rospy.Duration(self.pause_after_help)
            # Reset timers
            self._no_plan_accum = 0.0
            self._last_moving_time = now
...
if __name__ == "__main__":
    try:
        ...
        pass
.get_param("~plan_check_period", 1.0)
